...
/* ============================================================
   Fotos de producto → IndexedDB (no localStorage).
   localStorage tiene un límite chico (~5-10 MB) compartido entre
   todos los datos; las fotos pueden llenarlo rápido con pocos
   productos. IndexedDB soporta muchísimo más espacio, así que las
   fotos viven ahí, guardadas por id de producto.
   ============================================================ */
const IMG_DB_NAME = 'tapstock_imagenes_db';
const IMG_STORE = 'imagenes';

function _abrirImagenesDB() {
...
const ImagenesDB = {
  async guardar(id, dataUrl) {
    try {
      const db = await _abrirImagenesDB();
  ...
  async eliminar(id) {
    ...
    } catch (err) { console.error('No se pudo eliminar la imagen:', err); }
  }
};

const Store = {
  // ---------- Escritura segura (detecta límite de almacenamiento lleno) ----------
  _setItem(key, value) {
    try {
  ...
  saveProductos(lista) {
    return this._setItem(DB_KEYS.PRODUCTOS, JSON.stringify(lista));
  },
  getProducto(id) {
  ...
  upsertProducto(producto) {
    ...
    return this.saveProductos(lista);
  },
  eliminarProducto(id) {
    return this.saveProductos(this.getProductos().filter(p => p.id !== id));
  },
  ...
  saveCompras(lista) {
    return this._setItem(DB_KEYS.COMPRAS, JSON.stringify(lista));
  },
  ...
  saveFiados(lista) {
    return this._setItem(DB_KEYS.FIADOS, JSON.stringify(lista));
  },
  ...
  saveVentas(lista) {
    return this._setItem(DB_KEYS.VENTAS, JSON.stringify(lista));
  },
  ...
  saveConfig(cfg) {
    return this._setItem(DB_KEYS.CONFIG, JSON.stringify(cfg));
  },
  ...
  /* Busca en el HTML ya dibujado los .cap-wrap[data-pid] de productos
     cuya foto vive en IndexedDB (no inline) y las va insertando a
     medida que llegan, sin bloquear el resto del renderizado. */
  async inyectarFotosEnDOM(productos) {
    const pendientes = (productos || []).filter(p => !p.imagen && p.tieneImagen);
    for (const p of pendientes) {
      const src = await ImagenesDB.obtener(p.id);
      if (!src) continue;
      const wrap = document.querySelector(`.cap-wrap[data-pid="${p.id}"]`);
